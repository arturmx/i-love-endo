...
class DotPay {
    ...
    /**
     * Geenerate dotPay link|form
     * 
     * @param int $DotpayId
     * @param int $DotpayPin
     * @param string $Environment
     * @param string $RedirectionMethod
     * @param array $ParametersArray
     * @param array|false $MultiMerchantList
     * @return string
     */
    public function dotPayGenerateChkRedirection ($DotpayId, $DotpayPin, $Environment, $RedirectionMethod, $ParametersArray, $MultiMerchantList = array()) 
    {
        $ParametersArray['id'] = $DotpayId;
        $ChkParametersChain = $this->dotPayGenerateChk ($DotpayId, $DotpayPin, $Environment, $RedirectionMethod, $ParametersArray, $MultiMerchantList);
        $ChkValue = hash('sha256',$ChkParametersChain);
        
        // check Environment
        if ($Environment == 'production') {
            $EnvironmentAddress = 'https://dproxy.przelewy24.pl/t2/';
        } elseif ($Environment == 'test') {
            $EnvironmentAddress = 'https://dproxy.przelewy24.pl/test_payment/';
        }
        ...
        if ($RedirectionMethod == 'POST') 
        {
            ...
            '<button id="dotpay_redirection_button" type="submit" form="dotpay_redirection_form" class="button-default d-none"
            value="Submit">Confirm and Pay</button>'.PHP_EOL;
    ...
    * @param string $RedirectionMethod
    * @param array $ParametersArray
    * @param array $MultiMerchantList
    * @return string
    */
    private function dotPayGenerateChk ($DotpayId, $DotpayPin, $Environment, $RedirectionMethod, $ParametersArray, $MultiMerchantList = array())
    {
        $ParametersArray['id'] = $DotpayId;
        $chk = $DotpayPin.
